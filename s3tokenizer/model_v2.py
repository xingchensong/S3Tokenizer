...
class FSMNMultiHeadAttention(MultiHeadAttention):
    ...
    def qkv_attention(self,
        ...
        # NOTE(Mddct): S3tokenize if fuall attention, mask shape is [B,1,T]
        #  mask pad for fsmn can use this mask
        fsm_memory = self.forward_fsmn(v, mask)
    ...
    def forward(self,
        ...
        wv, qk, fsm_memory = self.qkv_attention(q, k, v, mask, freqs_cis)
...
class ResidualAttentionBlock(torch.nn.Module):
    ...
    def forward(
        ...
        x = x + self.attn(self.attn_ln(x), mask=mask, freqs_cis=freqs_cis)[0]
...
class AudioEncoderV2(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        for block in self.blocks:
            x = block(x, mask.unsqueeze(1), freqs_cis[:x.size(1)])
