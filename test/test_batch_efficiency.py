...
def test_batch_efficiency(test_audios, model_name):
    ...
    for i, audio in enumerate(test_audios):
        ...
        print(f"Audio {i+1}: {duration:.1f}s, {len(final_codes)} tokens, {processing_type}")
    ...
    for i in range(len(test_audios)):
        if individual_results[i] != batch_results[i]:
            print(f"❌ Audio {i+1} results don't match!")
            all_match = False
        else:
            print(f"✅ Audio {i+1} results match")
    ...
    assert all_match, f"Results don't match for model {model_name}"
    assert len(individual_results) == len(batch_results), "Number of results don't match"
    assert all(len(individual_results[i]) == len(batch_results[i]) for i in range(len(test_audios))), "Token counts don't match"
...
def test_pure_long_audio_batch(long_audios, model_name):
    ...
    print(f"Batch processing {len(long_audios)} long audios took: {processing_time:.2f}s")
    ...
    print(f"✅ Pure long audio batch processing test completed for {model_name}")
    ...
    assert len(results) == len(long_audios), "Number of results should match number of input audios"
    assert all(tokens_count > 0 for _, tokens_count in results), "All audio should produce tokens"
...
def test_single_audio_processing(model_name):
    ...
    assert codes is not None, f"Codes should not be None for model {model_name}"
    assert codes_lens is not None, f"Codes lengths should not be None for model {model_name}"
    assert len(final_codes) > 0, f"Should produce tokens for model {model_name}"
    assert codes_lens[0].item() == len(final_codes), f"Codes length should match actual codes for model {model_name}"

    duration = audio.shape[0] / 16000
    print(f"✅ Single audio processing test completed for {model_name}: {duration:.1f}s → {len(final_codes)} tokens")
